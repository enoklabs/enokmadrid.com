$(document).ready(function(){
    ...
    // PROCESS STEP TIMELINE ANIMATION COPY
    var timelineBlocks = $('.process-step');
    var offset = 0.8;
    ...
    function hideBlocks(blocks, offset) {
      blocks.each(function(){
        ( $(this).offset().top > $(window).scrollTop()+$(window).height()*offset ) && $(this).find('.process-info, .process-about').addClass('is-hidden');
      });
    }

    function showBlocks(blocks, offset) {
      blocks.each(function(){
        ( $(this).offset().top <= $(window).scrollTop()+$(window).height()*offset && $(this).find('.process-info').hasClass('is-hidden') ) && $(this).find('.process-info, .process-about').removeClass('is-hidden').addClass('bounce-in');
      });
    }
